...
public class IwUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String nombre){
    	try {
    		Usuario u = entityManager.createNamedQuery("Usuario.byNombreUsuario", Usuario.class)
                    .setParameter("nombre", nombre)
                    .getSingleResult();
	        // build UserDetails object
	        ArrayList<SimpleGrantedAuthority> roles = new ArrayList<>();
	        roles.add(new SimpleGrantedAuthority("ROLE_" + u.getRol()));
		    log.info("Roles for " + nombre + " include " + roles.get(roles.size()-1));
	       return new org.springframework.security.core.userdetails.User(
	        		u.getNombre(), u.getContraseña(), roles); 
	    } catch (Exception e) {
